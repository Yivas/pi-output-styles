`pi-output-styles` is open source collaborative software. The repository contains an executable extension, published as release `v0.2.0` on GitHub and as the npm package `pi-response-styles` version `0.2.0`, its first npm release. The verified Pi compatibility baseline is `0.87.0`; other Pi versions have not been run.
