`pi-output-styles` is open source collaborative software. The repository contains an executable extension, published as the GitHub release `v0.1.0`; the npm package `pi-response-styles` has no published version until `0.2.0`. The verified Pi compatibility baseline is `0.87.0`; other Pi versions have not been run.
