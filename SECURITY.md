...
The supported version is `0.5.1`. Security reports are accepted for that release and for repository commits after it; the only verified Pi compatibility baseline is `0.87.0`.

| Version or line | Support status |
| --- | --- |
| 0.5.1 | Supported; compatibility checked against Pi `0.87.0` only |
| Repository commits after 0.5.1 | Security reports accepted; not a numbered release |
...
- The version or commit tested: the release version (currently `0.5.1`) or the repository commit.
