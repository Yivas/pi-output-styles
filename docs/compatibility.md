...
- Package status: `pi-response-styles` `0.6.0` published on GitHub Releases as `v0.6.0` and on npm as `pi-response-styles@0.6.0`; compatibility verified against Pi `0.87.0` only; `waitingTurnReminder` blocked (FAIL-CLOSED); cross-plugin force delivery through `pi.events` covered by local integration tests
