...
- Package status: `pi-response-styles` `0.6.2` published on GitHub Releases as `v0.6.2` and on npm as `pi-response-styles@0.6.2`; compatibility verified against Pi `0.87.0` only; `waitingTurnReminder` blocked (FAIL-CLOSED); cross-plugin force delivery through `pi.events` covered by local integration tests
...
Version `0.4.0` added two TUI surfaces, covered by the local suite (`test/ui/style-menu.test.ts`, `test/ui/status-indicator.test.ts`, `test/commands.test.ts`, `test/integration/extension.test.ts`), which renders the component with a test theme at 40, 79, 80, and 120 columns. Version `0.5.2` moved the menu from the experimental overlay mode to Pi's native modal slot, described below:

- Opening `/output-style` with no arguments in a TUI session (`ctx.mode === "tui"`) opens the menu as a native dialog in the editor slot — the same place as Pi's own dialogs such as the model picker — with a `Select an output style` header and a navigable list whose rows carry a `>` cursor glyph and a `*` active glyph (the origin is shown only for custom styles) beside a detail panel with the description, the style metadata line (`turn reminder`, `keep-coding`, `waiting reminder: unavailable (Pi)` when Pi cannot deliver it, `from <path>` for custom styles), and the wrapped instruction body. Enter applies and persists the selection through the same path as `/output-style <id>`; Esc cancels without changes. `/output-style <id|name>` accepts the id or the name the menu shows, the id wins when both could match, and an ambiguous name is reported with the candidate ids. With the explicit `list` argument, and in RPC, JSON, and print modes, the command keeps the plain-text listing without dialogs.
...
- Below 80 render columns the detail panel collapses and the active row carries its description inline; the status line stays reserved at the bottom of the block and is dropped only when the row budget cannot hold it.
...
- Borders, optional banner, list, detail, and footer share one row budget, so the status line never renders past the rows the dialog paints; on terminals of five rows or fewer the menu keeps at least one style row and can exceed the screen cap by one row.
- The status bar shows `style: <name>` for the effective style under the extension-owned key `pi-output-styles`: muted for `default`, accent otherwise. It is painted in TUI mode at session start, after a direct selection, and after the menu's Enter, and it is repainted when a plugin applies or releases a forced style, so it always names the style Pi will apply. Colors come from the theme current at write time; a theme change does not repaint an already-written entry.
- Style content is untrusted text for the terminal: the menu, the plain listing, the status messages and the turn reminders strip terminal escape sequences (CSI, OSC, DCS and the other escape forms) and the invisible formatting that reorders text (bidi controls and the zero-width space) before showing them, while keeping the text the model receives untouched. The plugin does not claim the terminal is clean for content produced by other extensions.
...
- Style menu and status indicator in an interactive session: component renders were captured through Pi's real theme and the dialog path at 79 and 80 columns, while renders at 40 and 120 columns come from the local suite's test theme; the modal slot delivered in `0.5.2` has not been captured in a real TUI, and an interactive human pass over the live menu has not been run.
